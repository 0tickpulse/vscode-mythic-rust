...
import { Disposable, Executable, LanguageClient, LanguageClientOptions, MessageSignature, ServerOptions } from "vscode-languageclient/node";

let client: LanguageClient;
// type a = Parameters<>;

export async function activate(context: ExtensionContext) {
    const traceOutputChannel = window.createOutputChannel("Mythic Language Server trace");
    const command =
        process.env.SERVER_PATH ||
        context.asAbsolutePath(join("target", "debug", "mythic-language-server") + process.platform === "win32" ? ".exe" : "");
    console.log(`Connecting to language server at ${command}...`);
    ...
    let clientOptions: LanguageClientOptions = {
        ...
        documentSelector: [{ scheme: "file", language: "MythicYAML" }],
        synchronize: {
    ...
    client = new LanguageClient("mythic-language-server", "Mythic Language Server", serverOptions, clientOptions);
